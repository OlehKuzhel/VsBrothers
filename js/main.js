...
// Первый шаг
    ...
    $('body').on('click', '.link--diller', function(event) {
        ...
        $('.steps-progress__items').hide();
        if($id=="new")
        {
            $('.steps-progress__items_new').show();
        }
        if($id=="old")
        {
            $('.steps-progress__items_experienced').show();
        }
        $('.steps-content__navigation').eq(0).find('.link--next').trigger('click');
        /* Act on the event */
    });
    ...
    $('.steps-content__navigation').on('click', '.link--back', function(event) {
        ...
        if($activeNumb=="2")
        {
            $('.steps-content__navigation').eq(0).find('.link--next').css('opacity', "0");
            if($activeCheckbox=="old")
            {
                $('.steps-progress__items_experienced').hide();
                $('.steps-progress__items').eq(0).show();
            }

        }else
        {
            $('.steps-content__navigation').eq(0).find('.link--next').css('opacity', "1");
        }
        // console.log("activeNumb = "+$activeNumb);


        if ($activeNumb == 3) {
            if($activeCheckbox=="new")
            {
                $nextStep = $('.steps-content__item[data-step=1]')
                $('.steps-progress__items_new').hide();
                $('.steps-progress__items').eq(0).show();
                $('.steps-content__navigation').eq(0).find('.link--next').css('opacity', "0");
            }else
            {
    ...
    $('.steps-content__navigation').on('click', '.link--next:not(.not-active)', function(event) {
        ...
        $activeCheckbox = $(this).attr('data-type')
        if ($activeNumb != 3) {
            if($activeCheckbox=='new'&&$activeNumb == 2)
            {
